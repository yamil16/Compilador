...
//			System.out.println("reg " + nroRegistroTercetoOperando2);
	...
	public void GenerarAssemblerMultiplicacion(Terceto t, PrintWriter arc,String Operador)throws IOException{
...
//					System.out.println("registro obtenido: " + registro + " para suma");
		...
		//nuevo
			...
			Token t1= (Token) t.getOperando1();
			if (Compilador.T_simbolos.get(t1.getClaveTablaSimboloToken()).getToken() == 257)					
				arc.append(Operador+Compilador.T_simbolos.get(t1.getClaveTablaSimboloToken()).getUso()+operando1 + "\r\n");
			else{
				
					if(operando1.codePointAt(0)==95){
						String valor1=operando1.substring(2, operando1.length());
					  arc.append(Operador+" _in"+valor1 + "\r\n");
				  }
				  else
					  arc.append(Operador+" _i"+operando1 + "\r\n");
			...
			if(!t.getOperando2().isTerceto()){ //es un token
				...
				Token t2= (Token) t.getOperando2();
				if (Compilador.T_simbolos.get(t2.getClaveTablaSimboloToken()).getToken() == 257){					
					arc.append(Operador+Compilador.T_simbolos.get(t2.getClaveTablaSimboloToken()).getUso()+operando2 + "\r\n");
				}
				else{
	...
	public void GenerarAssemblerOperacion2daPrecedencia(Terceto t, PrintWriter arc,boolean Esdivision) throws IOException {
		...
		int nroRegistro=t.getRegistro();		
		String nombreRegistro=getNombreRegistro(nroRegistro);
		if (nombreRegistro=="A"){
			if(!DisponibleDX)
				arc.append("MOV DX, @auxDx"+ "\r\n");				
		}
		...
		System.out.println();
		System.out.println();
		System.out.println("Disponible Ax "+DisponibleAX+" Disponible Dx "+DisponibleDX+ "\r\n");
		System.out.println();		
	...
	public void GenerarAssemblerMultiplicacion(Terceto t, PrintWriter arc) throws IOException{
		if(t.getTipo()=="Integer"){				
			GenerarAssemblerMultiplicacion(t, arc,"IMUL ");	
		}
		else{ //NO ES DE TIPO INTEGER
			GenerarAssemblerMultiplicacion(t,arc,"MUL ");
		}
	}
...
//		System.out.println("terceto " + t.getLexema());
		switch(t.getOperador()){
			...
			case "/":{
				...
				//GenerarAssemblerDivision(t,arc);
			break;}
	...
	public void GenerarAssemblerDivision(Terceto t, PrintWriter arc) throws IOException{
		if(!t.getOperando1().isTerceto()){ //es un token
			if(!t.getOperando2().isTerceto()){ //es un token
					...
					//hago el chequeo de dvi por cero
						arc.append("MOV "+nombreRegDispobible+"X, "+operando2);
			
						arc.append("CMP "+nombreRegDispobible+"X, 0");
			
						arc.append("JE DIVCERO");	 
					
					
					if (registrosLibres[0]) { //esta dsponible el Ax
						t.setRegistro(0);
						registrosLibres[0] = false;
						if (registrosLibres[3]) {
							registrosLibres[3] = false;
								arc.append("MOV AX, "+operando1);
						
							
							Integer registro = obtenerRegistroLibre();
							String nombreReg = getNombreRegistro(registro);
							registrosLibres[3] = true;
								arc.append("MOV "+nombreReg+"X, "+operando2);
							

								arc.append("CWD"); //Extiende el signo
							
							
								arc.append("IDIV "+nombreReg+"X");
							...
							String nombreReg = getNombreRegistro(registro);
								...
								arc.append("MOV "+nombreReg+"X, @auxDx");
						...
						if (registrosLibres[3]) {
							...
							Integer regOp2 = obtenerRegistroLibre();
							String nombreRegOp2 = getNombreRegistro(regOp2);

							
								arc.append("MOV AX, "+operando1);
							
								arc.append("MOV "+nombreRegOp2+"X, "+operando2);
						
								arc.append("CWD");
							
								arc.append("IDIV "+nombreRegOp2+"X");
						
							
								arc.append("MOV @auxAx, AX");
						
								arc.append("MOV AX, "+nombreReg+"X");
			
								arc.append("MOV "+nombreReg+"X, @auxAx");
			...
			}//operando 2 es un terceto
			else{
				//busco el registro del tercero
				String lexemaTerceto=t.getOperando2().getLexemaGeneradorNumero();
				int nroterceto = (Integer.parseInt(lexemaTerceto.substring(1, lexemaTerceto.length() - 1)));
				int registroOperando2=listaTercetos.get(nroterceto).getRegistro();
				String nombreReg = getNombreRegistro(registroOperando2);
				...
				////hago el chequeo de dvi por cero pero ya se que el divisior ya se encuentra en un registro. 
					arc.append("CMP "+nombreReg+"X, 0");
				...
				if (registrosLibres[0]) {
					...
					if (registrosLibres[3]) {
						...
						Integer registro = obtenerRegistroLibre();
						String nombre = getNombreRegistro(registro);
						registrosLibres[registro] = false;
						...
						if (nombreReg.equals("A")) {
							...
							arc.append("MOV AX, "+nombre+"X");
					
							arc.append("MOV "+nombre+"X, @auxAx");
